...
class Body():
    def __init__(self, body_elem, parent=None):
        ...
        elif self.geom_type == 'capsule' or self.geom_type == 'cylinder':
    ...
    def create_skeleton(self, local_frame = np.array([0,0,0.])):
        if self.need_auxilary_bones():
            # for example, pelvis has 3 adjcent hips, we need to insert skeletons to feed the space
            # just as cmu mocap's fbx model, we use a tiny bone to approximate a shpere joint.
            self._draw_skeleton(
                self.name, 
                self.pos + local_frame, 
                1.001*self.pos + local_frame,
                self.parent_name)
            for body in self.children:
                ...
                self._draw_skeleton(
                    aux_name,
                    self.pos + local_frame,
                    body.pos + self.pos + local_frame,
                    self.name
                )
                body.parent_name = aux_name
        elif self.is_leaf():
            # add end bone. the tail is not bind to any other one, so has to assign a length manully
            self._draw_skeleton(self.name, self.pos + local_frame, self.pos + local_frame + self.bone_length, self.parent_name)
        else:
            self._draw_skeleton(self.name, self.pos + local_frame, self.children[0].pos + self.pos + local_frame, self.parent_name)
...
def parse_mjcf(file_path):
    ...
    def traverse_bodies(body_elem, parent=None):
        ...
        for child in body_elem.findall('body'):
            child_info = traverse_bodies(child, parent=body_info.name)
...
class RokokoMotionRetarget():
    ...
    def export_to_json(self, filename):
        ...
        for i, bone in enumerate(bpy.context.scene.rsl_retargeting_bone_list):
            data.append({
                "source": bone.bone_name_source,
                "target": bone.bone_name_target,
                "rsl_retargeting_bone_list_id": i
            })
    ...
    def import_from_json(self, filename):
        ...
        for map in data:
            ...
            assert bpy.context.scene.rsl_retargeting_bone_list[i].bone_name_source == map['source'], f"Bone name mismatch: {bpy.context.scene.rsl_retargeting_bone_list[i].bone_name_source} != {map['source']}"
    ...
    def print_bone_trees(self, filepath):
        def print_bone_hierarchy(bone, file, indent=0):
            ...
            if len(bone.children) > 0: file.write(":\n")
            else : file.write("\n")
...
mjcf_path = "C:\\Users\\m1300\\Desktop\\smplx.xml"
body_hierarchy = parse_mjcf(mjcf_path)
body_hierarchy[0].t2fbx("C:/Users/m1300/Desktop/out.fbx")

# mr = RokokoMotionRetarget("Armature","MJCF_Armature")
# mr.print_bone_trees("C:\\Users\\m1300\\Desktop\\tree.yaml")
# mr.export_to_json("C:\\Users\\m1300\\Desktop\\bone_map.json")
# mr.import_from_json("C:\\Users\\m1300\\Desktop\\bone_map.json")
